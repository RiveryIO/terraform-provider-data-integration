...
3. **Terraform provider — MVP:** the provider now lives at the **repo root** (Go +
   terraform-plugin-framework), client ported from `rivery_client.py`; resources
   `boomi_data_integration_environment` → `..._connection` → `..._data_flow` with full CRUD +
   `import`; unit tests + `TF_ACC` acceptance tests + examples + Registry docs. ✅ (`data_flow`
   verified live against integration; see `../README.md`). See `design/CORE-2346-comparison.md`.
