...
templates/                           tfplugindocs templates + guides (docs SOURCE — edit these)
docs/                                Terraform Registry documentation (GENERATED — do not edit)
.goreleaser.yml                      signed multi-platform release build
```

## Documentation authoring

`docs/` is generated by `make docs`; never edit it by hand. The source of truth is
`templates/`:

| Path | Renders to | Notes |
| --- | --- | --- |
| `templates/resources/<short_name>.md.tmpl` | `docs/resources/<short_name>.md` | `<short_name>` is the resource type with the provider local name stripped: `boomi_data_integration_data_flow` → `data_integration_data_flow`. A wrong filename **silently does nothing** — tfplugindocs falls back to its built-in default template. |
| `templates/data-sources/<short_name>.md.tmpl` | `docs/data-sources/<short_name>.md` | Same rule. |
| `templates/guides/<name>.md` | `docs/guides/<name>.md` | Files that do not end in `.tmpl` are copied verbatim, so guides are plain Markdown. The registry page slug is the filename without its extension, and guides need `page_title` in their front matter. |

Resources and data sources with no template here render with the built-in default, so adding one
file affects exactly one page. `docs/resources`, `docs/data-sources`, `docs/guides` and
`docs/index.md` are *managed* by tfplugindocs — deleted and rebuilt on every run — which is why
guides live in `templates/`. `docs/artifacts/` is not managed and is left alone.

Where prose goes:

- **Attribute `Description` in the Go schema**: 1–3 sentences stating the contract, plus a pointer
  to a guide. Descriptions render as a single bullet in the registry's attribute list, so anything
  longer makes the list unscannable — that is how `properties_json` once grew to ~400 words.
- **Page body** (`templates/resources/*.md.tmpl`): lifecycle behaviour and cross-attribute rules.
- **Guides** (`templates/guides/*.md`): recipes, wire shapes, enums, traps.

Cross-page links use extension-less relative paths, e.g.
`[Incremental extraction](../guides/incremental-extraction)` from a resource page.

Template functions available in v0.21.0: `codefile`, `tffile`, `plainmarkdown`, `prefixlines`,
`lower`, `upper`, `title`, `split`, `trimspace`. Resource templates receive `.Name`, `.Type`,
`.Description`, `.SchemaMarkdown`, `.HasExample`/`.ExampleFile`, `.HasImport`/`.ImportFile`,
`.ProviderName`, `.ProviderShortName`, `.RenderedProviderName`.
