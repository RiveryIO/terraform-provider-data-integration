...
#
# Column metadata for regular Jira flows is stored in
# target.single_table_settings.mapping (NOT in schemas.tables — the API
# rejects run_type_and_datasource = "regular" there).
# The default 147-column schema matches the standard Jira "issue" report layout.
...
module "dataflow" {
  ...
  target = {
    ...
    single_table_settings = var.columns != null ? {
      mapping = var.columns
    } : null
  }
