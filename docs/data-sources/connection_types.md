# boomi_connection_types (Data Source)
...
`boomi_connection`, and pair it with the `boomi_connection_type` data source to
...
output "available_types" {
  value = [for t in data.boomi_connection_types.all.connection_types : t.connection_type]
}
```
...
- `connection_types` (List of Object) — All available connection types, sorted by `connection_type`. Each element has:
  - `connection_type` (String) — The identifier used as `boomi_connection.type` (e.g. `mysql`).
